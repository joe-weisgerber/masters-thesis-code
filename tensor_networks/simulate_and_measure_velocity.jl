...
function generate_data(L, T, tau, g, p, gauge_breaking_sites, gauge_breaking_strengths, cb, maxdim, cutoff, stop, folder)
    ...
    if g!=0
        H = hamiltonian_middle_perturbations(1, L, sites; strength=g, type="cc_breaking")
    elseif p!=0
        H = hamiltonian_middle_perturbations(1, L, sites; strength=p, type="potential")
    else
    ...
    results = thermalize_and_measure_velocity(Ψ, H, tau, T, p, g, gb, gauge_breaking_sites, L, sites, folder; maxdim=maxdim, cutoff=cutoff, stop=stop)
    
    h5open("$(folder)/L=$(L)_g=$(g)_p=$(p)_gb=$(gb)_T=$(T)_tau=$(tau)_cutoff=$(cutoff)_maxdim=$(maxdim)_stop=$(stop)_velocity_measurements.h5", "w") do file
        for k in keys(results)
            write(file, k, results[k])
        end
    end
    
    println("Data saved as $(folder)/L=$(L)_g=$(g)_p=$(p)_gb=$(gb)_T=$(T)_tau=$(tau)_cutoff=$(cutoff)_maxdim=$(maxdim)_stop=$(stop)_velocity_measurements.h5")
    println("------------------------------------------------------")
end
