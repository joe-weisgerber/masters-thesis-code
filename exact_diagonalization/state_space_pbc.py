...
def height_rep_to_physical(state, pbc=True):
    if pbc :
...
def apply_U_plaquette(state, site, s, L):
...
def apply_U_plaquette_dagger(state, site, s, L):
    # Apply the plaquette term at a given site
    # State is in tight representation
...
def gauge_breaking_term(state, site, s, L):
...
def gauge_breaking_term_dagger(state, site, s, L):
    # Apply gauge breaking term on given site
    # State is in tight representation
...
def construct_gauge_sector(s, L):
...
def construct_gauge_broken_sector(s, L, gauge_breaking_sites):
